...
async def search_users(q: str = ""):
    ...
    counts = await asyncio.gather(*[
        recipes_collection.count_documents({"user_id": u["_id"], "is_public": True, "is_modified": {"$ne": True}, "original_recipe_id": {"$exists": False}})
        for u in users
    ] + [
...
async def browse_users():
    ...
    results = await asyncio.gather(*[
        ...
        recipes_collection.find_one(
            {"user_id": u["_id"], "is_public": True, "is_modified": {"$ne": True}, "original_recipe_id": {"$exists": False}, "image_url": {"$exists": True, "$gt": ""}},
            {"image_url": 1}
        )
        for u in users
    ])
...
async def get_user_public_recipes(user_id: str):
    ...
    cursor = recipes_collection.find({"user_id": oid, "is_public": True, "is_modified": {"$ne": True}, "original_recipe_id": {"$exists": False}})
    results = []
...
async def get_user(user_id: str):
    ...
    follower_count, following_count, recipe_count = await asyncio.gather(
        ...
        recipes_collection.count_documents({"user_id": user["_id"], "is_public": True, "is_modified": {"$ne": True}, "original_recipe_id": {"$exists": False}}),
    )
    return {
        "id": uid,
        "name": user.get("name"),
